# 文件名: run_entropy_incremental_milestones.py
...
def main(args):
    ...
    milestone_logger, _, _ = get_logfile(args.ckpt_path, args.exp_name, checkpointer=False, snapshot=None,
                                         log_name='entropy_milestone_log.txt')
    ...
    print(f"\n--- 开始基于熵的增量学习循环，共 {num_al_steps} 轮，直到达到 50% 的数据量 ---")

    for i in range(num_al_steps):
        num_labeled_before = train_set.get_num_labeled_videos()
        print(f'\n----- 熵选择第 {i + 1}/{num_al_steps} 轮: 当前已标注 {num_labeled_before}/{max_budget} -----')
        ...
        num_to_select = min(args.num_each_iter, len(action_indices))
        ...
        selected_indices = [candidate_indices[idx] for idx in action_indices[:num_to_select].tolist()]
    ...
    print("-" * 50)
    print(f"详细日志已保存至: {os.path.join(args.ckpt_path, args.exp_name, 'entropy_milestone_log.txt')}")
